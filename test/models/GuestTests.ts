...
import {
  invalidGuestLengthError,
  duplicateGuestError,
} from '../../src/models/validators/errors';
  ...
  describe('#guests', () => {
    it('must be fewer than maxSize', (done) => {
      ...
      party.validate((err) => {
        ...
        expect(err.errors.guests.message).to.equal(
          invalidGuestLengthError.message(party.maxSize)
        );
        done();
      });
    });
    ...
    it('must be unique across documents', (done) => {
      PartyModel.create<Partial<Party>>(
        { maxSize: 1, guests: [{ name: 'Aram' }] },
        () => {
          ...
          p.save((err) => {
            expect(err.errmsg).to.match(
              /duplicate key error collection: [^.]+\.parties index: guests\.name/
            );
            done();
          });
        }
    ...
    it('must be unique within document', (done) => {
      party.maxSize = 2;
      party.guests = [{ name: 'Aram' }, { name: 'Aram' }];
      party.validate((err) => {
        expect(err.errors.guests.kind).to.equal(duplicateGuestError.kind);
        expect(err.errors.guests.message).to.equal(
          duplicateGuestError.message([{ name: 'Aram' }])
        );
        done();
      });
    });
